...
export default function AuthPage() {
  ...
  const submitSignUp = async () => {
    ...
    await refreshState();
    setMessage("注册请求已提交，请按你的 Supabase 配置完成邮箱验证后登录。");
    setPending(false);
  };
  ...
  return (
    <main style={{ padding: 24, display: "grid", gap: 24 }}>
      <h1>认证与运行时配置</h1>
      <p>当前会话：{sessionEmail ?? "未登录"}</p>
      {!client ? <p>当前环境未配置 NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY。</p> : null}
      {message ? <p>{message}</p> : null}

      <section aria-label="supabase-auth" style={{ display: "grid", gap: 12 }}>
        <h2>Supabase 登录</h2>
        <form onSubmit={submitSignIn} style={{ display: "grid", gap: 8, maxWidth: 420 }}>
          <label>
            Email
            <input
              type="email"
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button type="button" onClick={() => void submitSignUp()} disabled={pending}>
              注册
            </button>
      ...
      <section aria-label="manual-runtime" style={{ display: "grid", gap: 12 }}>
        <h2>手动运行时（联调用）</h2>
        <form onSubmit={submitManual} style={{ display: "grid", gap: 8, maxWidth: 620 }}>
          <label>
            Access Token
            <textarea
              value={manualToken}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button type="submit">保存手动配置</button>
            <button type="button" onClick={() => void clearManual()}>
              清空手动配置
            </button>
          </div>
        </form>
